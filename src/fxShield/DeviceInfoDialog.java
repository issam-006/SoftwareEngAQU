...
public final class DeviceInfoDialog {
    ...
    private static Info collectInfo() {
        try {
            ...
            info.cpuLoad = (load == null) ? -1 : (load * 100.0);
            ...
            // GPU clocks (best-effort NVIDIA via nvidia-smi)
            try {
                if (!info.gpus.isEmpty()) {
                    String out = runCmd("nvidia-smi",
                    ...
                    for (int i = 0; i < info.gpus.size(); i++) {
                        ...
                        if (i < lines.length) {
                            ...
                            if (parts.length >= 2) {
                                g.coreClock = parts[0].trim() + " MHz";
                                g.memClock  = parts[1].trim() + " MHz";
                            }
                        }
                        if (isNA(g.coreClock)) g.coreClock = "N/A";
                        if (isNA(g.memClock))  g.memClock  = "N/A";
                    }
                }
